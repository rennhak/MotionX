...
# use tor + privoxy with cmd tools
...
# == Jukebox is the Main Class handling everything of this project.
class Jukebox
    def initialize
        ...
        @music    = Music.new                                           # Create Music interface
    end
...
optionParser = OptionParser.new do |opts|
    ...
    opts.on("-r", "--recache", "Recache the Playlist") do |r|
        @@options[:recache] = r
    end
    ...
    @@options[:version] = v
        ...
        print "Copyright (c) "
        ( DateTime.now.year == 2007 ) ? ( print DateTime.now.year ) : ( print "2007-#{DateTime.now.year.to_s}" )
        puts " #{@@author}"
        exit
    end
end
...
begin
    ...
    jukebox = Jukebox.new                                               # Fire up Jukebox
